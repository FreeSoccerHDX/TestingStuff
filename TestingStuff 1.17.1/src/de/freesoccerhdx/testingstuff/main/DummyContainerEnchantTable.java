...
public class DummyContainerEnchantTable extends Container {

	protected DummyContainerEnchantTable(@Nullable Containers<?> containers, int i) {
		super(containers, i);
...
/*
	if(cmd.equalsIgnoreCase("/enchtest")) {
		
		CraftPlayer cp = (CraftPlayer) p;
		EntityPlayer ep = cp.getHandle();
		EntityHuman eh = ep;
		
		net.minecraft.server.v1_16_R3.World nmsworld = eh.world;
		IBlockData blockdata = Blocks.ENCHANTING_TABLE.getBlockData();
		BlockPosition blockposition = new BlockPosition(-8, 73, 366);
		
		TileEntity tileentity = nmsworld.getTileEntity(blockposition);
		IChatBaseComponent chatcompo = ((INamableTileEntity)tileentity).getScoreboardDisplayName();
		
		
		
		TileInventory ti = new TileInventory((var2, var3, var4) -> new DummyContainerEnchantTable(var2, var3, ContainerAccess.at(nmsworld, blockposition)), chatcompo);
		
		eh.openContainer(ti);
		
		//ContainerEnchantTable cet = new ContainerEnchantTable(-1, ep.inventory);
    ...
    public DummyContainerEnchantTable(int i, PlayerInventory playerinventory) {
        this(i, playerinventory, ContainerAccess.a);
    }

    public DummyContainerEnchantTable(int i, PlayerInventory playerinventory, ContainerAccess containeraccess) {
        super(Containers.ENCHANTMENT, i);
        this.enchantSlots = new InventorySubcontainer(2) {
            @Override
            public void update() {
                super.update();
                DummyContainerEnchantTable.this.a((IInventory) this);
            }
                    ...
                    for (Map.Entry<org.bukkit.enchantments.Enchantment, Integer> entry : event.getEnchantsToAdd().entrySet()) {
                        try {
                            if (flag) {
                                ...
                                net.minecraft.server.v1_16_R3.Enchantment nms = IRegistry.ENCHANTMENT.get(CraftNamespacedKey.toMinecraft(enchantId));
                                if (nms == null) {
                                    continue;
                                }
